...
namespace {
  ...
  public:
    ...
    unsigned initHookArgs(std::string s, Instruction *I, Function *F, std::vector<Value*> &args, unsigned offset=0) {
        LLVMContext& C = I->getContext();
        Type* i128Ty = Type::getInt128Ty(C);
        Type* i64Ty = Type::getInt64Ty(C);
        int i = 1;   
        ...
        Value * ae = builder.CreateGlobalString(StringRef(s),"varName");     
        for (auto V : I->operand_values()) {
            i++;
            FunctionType *FT = F->getFunctionType();
            Type *T = offset < FT->getNumParams() ? FT->getParamType(offset) : NULL;    
            if (i==2){args.push_back(createCast(ae, T, I));
                offset++; break;}   
            if (!T) {
                if (F->isVarArg())
                    args.push_back(V);              
 ...
 unsigned initHookArgmain(Instruction *I, Function *F, std::vector<Value*> &args, unsigned offset=0) {
        LLVMContext& C = I->getContext();
        Type* i128Ty = Type::getInt128Ty(C);
        Type* i64Ty = Type::getInt64Ty(C);
        int i = 1;
 ...
 unsigned initHookArgcall(Instruction *I, Function *F, std::vector<Value*> &args, unsigned offset=0) {
        LLVMContext& C = I->getContext();
        Type* i128Ty = Type::getInt128Ty(C);
        Type* i64Ty = Type::getInt64Ty(C);
        int i = 0;
       ...
       while(i<=5){        
        //  Instruction *NextI = NextI -> getNextNonDebugInstruction();
        FunctionType *FT = F->getFunctionType();
        Type *T = offset < FT->getNumParams() ? FT->getParamType(offset) : NULL;
        BasicBlock *BB = I->getParent();
        IRBuilder<> builder(BB);
        std::string str = "ss";
        Value * ae = builder.CreateGlobalString(StringRef(str),"varName");
        args.push_back(createCast(ae, T, I));
        offset++;
        i++;}
        return offset;
    }

 unsigned initHookArgload(Instruction *I, Function *F, std::vector<Value*> &args, unsigned offset=0) {
        LLVMContext& C = I->getContext();
        Type* i128Ty = Type::getInt128Ty(C);
        Type* i64Ty = Type::getInt64Ty(C);
        int i = 1;
  ...
  void wrapHook(Instruction *I, Function *F) {
        ...
        offset = initHookArgmain(I, F, args, offset);
    ...
    void preHook(std::string s, Instruction *I, Function *F) {
        std::vector<Value*> args;  
        unsigned offset = initHookBaseArgs(I, F, args);   
        offset = initHookArgs(s, I, F, args, offset);
    ...
    void postHook(Instruction *I, Function *F) {
        std::vector<Value*> args;  
        unsigned offset = initHookBaseArgs(I, F, args);    
        offset = initHookArgcall(I, F, args, offset);
        FunctionType *FT = F->getFunctionType();
        CallInst *CI = CallInst::Create(F, args, "", I); 
        hookInlineAsNeeded(CI);
    }

    void preHookload(Instruction *I, Function *F) {   
        std::vector<Value*> args;  
        unsigned offset = initHookBaseArgs(I, F, args);      
        offset = initHookArgload(I, F, args, offset);
        if (offset == 100){return;}  
        FunctionType *FT = F->getFunctionType();
        CallInst *CI = CallInst::Create(F, args, "", I); 
        hookInlineAsNeeded(CI);
 ...
 unsigned initHookArgret(Instruction *I, Function *F, std::vector<Value*> &args, unsigned offset=0) {
        LLVMContext& C = I->getContext();
        Type* i128Ty = Type::getInt128Ty(C);
        Type* i64Ty = Type::getInt64Ty(C);
        int i = 1;
        for (auto V : I->operand_values()) {
            if(!V->getType()->isIntOrIntVectorTy()){break;}
            i++;
            FunctionType *FT = F->getFunctionType();
            Type *T = offset < FT->getNumParams() ? FT->getParamType(offset) : NULL;
            if (!T) {
    ...
    void preHookret(Instruction *I, Function *F) {   
        std::vector<Value*> args;  
        unsigned offset = initHookBaseArgs(I, F, args);         
        offset = initHookArgret(I, F, args, offset); 
        FunctionType *FT = F->getFunctionType();
        CallInst *CI = CallInst::Create(F, args, "", I); 
        hookInlineAsNeeded(CI);
    }
    ...
    void hook(Instruction *I) {        
        ...
        if (CS.getInstruction()) {
            ...
            if (Callee) {
                if (CS.getIntrinsicID() == Intrinsic::ID::not_intrinsic){
                    printf("---------------------------\n");
                    na = Callee->getName().str();
                    printf("%s\n", Callee->getName().str().c_str());
                    if (na == "__main" or na == "read" or na == "fread" or na == "pread")
                         {suffix = "call_" + na;}
                    else {
                    suffix = "call";}}
        ...
        if(suffix == "load") {
            Function *R =  I->getParent()->getParent();      
            std::string str = R->getName().str();
            std::string nan;
            if(str.find(".")!= std::string::npos){
             nan = str.substr(0, str.find_first_of("."));
             preHook(nan, I, Hooks::getFunction(H->Post, I->getType(), H->PostTypeMap));}
            else{preHook(str, I, Hooks::getFunction(H->Post, I->getType(), H->PostTypeMap));}
            preHookload(I, Hooks::getFunction(H->Pre, I->getType(), H->PreTypeMap));
        }

        if(suffix == "ret") {
            Function *Rs =  I->getParent()->getParent();      
            std::string strr = Rs->getName().str();
            std::string nan;
            if(strr.find(".")!= std::string::npos){
             nan = strr.substr(0, strr.find_first_of("."));
             preHook(nan, I, Hooks::getFunction(H->Post, I->getType(), H->PostTypeMap));}
            else{preHook(strr, I, Hooks::getFunction(H->Post, I->getType(), H->PostTypeMap));}         
            preHookret(I, Hooks::getFunction(H->Pre, I->getType(), H->PreTypeMap));
        }
        
        if(suffix == "call") {
        std::string nan;
        if(na.find(".")!= std::string::npos){
        nan = na.substr(0, na.find_first_of("."));
        preHook(nan, I, Hooks::getFunction(H->Pre, I->getType(), H->PreTypeMap));}
        else{
        preHook(na, I, Hooks::getFunction(H->Pre, I->getType(), H->PreTypeMap));}

        postHook(I, Hooks::getFunction(H->Post, I->getType(), H->PostTypeMap));
        }

        if (suffix == "call___main" )
        //postHook(I, Hooks::getFunction(H->Post, I->getType(), H->PostTypeMap));
